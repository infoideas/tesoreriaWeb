...
public class BeanBase {
    ...
    public static int CONCEPTO_COBRANZAS=1;
    public static int CONCEPTO_GASTOS=2;
    ...
    public static int getEFECTIVO() {
        return EFECTIVO;
    }

    public static void setEFECTIVO(int EFECTIVO) {
        BeanBase.EFECTIVO = EFECTIVO;
    }

    public static int getCONCEPTO_COBRANZAS() {
        return CONCEPTO_COBRANZAS;
    }

    public static void setCONCEPTO_COBRANZAS(int CONCEPTO_COBRANZAS) {
        BeanBase.CONCEPTO_COBRANZAS = CONCEPTO_COBRANZAS;
    }

    public static int getCONCEPTO_GASTOS() {
        return CONCEPTO_GASTOS;
    }

    public static void setCONCEPTO_GASTOS(int CONCEPTO_GASTOS) {
        BeanBase.CONCEPTO_GASTOS = CONCEPTO_GASTOS;
    }
    ...
    public Empresa cargaEmpresa(int idEmpresa){
    ...
    public AsientoRealizado realizaAsiento(Asiento asiento) {
            ...
            System.out.println("Fecha de mov api: " + asiento.getFecMov());
    ...
    //Obtiene moneda
    public Moneda obtieneMoneda(int codMoneda){
            Moneda moneda = null;
            Session session = null;
            try{
                session=HibernateUtil.getSessionFactory().openSession();
                session.beginTransaction();
                moneda=(Moneda) session.get(Moneda.class,codMoneda);
                session.getTransaction().commit();
            }
    ...
    public FormaPago obtieneFormaPago(int codFP){
            FormaPago forma = null;
            Session session = null;
            try{
                session=HibernateUtil.getSessionFactory().openSession();
                session.beginTransaction();
                forma=(FormaPago) session.get(FormaPago.class,codFP);
                session.getTransaction().commit();
            }
            catch (HibernateException e){
                session.getTransaction().rollback();
    ...
    //Obtiene el saldo anterior de la cuenta de fondos
    public double obtenerSaldo(CuentaFondos cuenta,Date fecha,int idFormaPago){
        CallableStatement s=null;
        ResultSet r=null;
        int li_id;
        double ld_saldo = 0;
        ...
        try {
               ...
               s=conexion.prepareCall("{call sp_get_saldo_tesoreria ( ? , ? , ? )}",ResultSet.TYPE_SCROLL_SENSITIVE,ResultSet.CONCUR_READ_ONLY);
               s.setInt(1,cuenta.getId());
               java.sql.Date lda_fecha;
               lda_fecha=new java.sql.Date(fecha.getTime());
               s.setDate(2,lda_fecha);
               s.setInt(3,idFormaPago);
